...
public class MSField {
    ...
    /**
     * Reads a minesweeper field from a text file and constructs an
     * representation of that field.
     * 
     * @param path
     *            the path to the file that represents a mine sweeper field.
     */
    ...
    /**
     * Uncovers a position x,y and returns the number of mines in the
     * neighborhood.
     * 
     * @param x
     * @param y
    ...
    private int countNeighbors(int x, int y) {
        int count = 0;
        if (validPosition(x + 1, y + 1) && f[x + 1][y + 1])
            count++;
        if (validPosition(x + 1, y) && f[x + 1][y])
            count++;
        if (validPosition(x + 1, y - 1) && f[x + 1][y - 1])
            count++;
        if (validPosition(x, y + 1) && f[x][y + 1])
            count++;
        if (validPosition(x, y - 1) && f[x][y - 1])
            count++;
        if (validPosition(x - 1, y + 1) && f[x - 1][y + 1])
